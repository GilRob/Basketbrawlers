...
glm::ivec3 Framebuffer::_MaxFramebufferSize; // Width, Height, and Layers (Depth) texture dimensions
...
void Framebuffer::init()
{
	...
	{
		_ResolutionUBO.allocateMemory(sizeof(glm::vec4));
	}
	_ResolutionUBO.sendVector(glm::vec4((float)_Width, (float)_Height, 1.0f / _Width, 1.0f / _Height), 0);
	...
	if (_DepthActive)
	{
		...
		glTextureParameteri(_Depth._Tex._Handle, GL_TEXTURE_MIN_FILTER, _Filter);
		glTextureParameteri(_Depth._Tex._Handle, GL_TEXTURE_MAG_FILTER, _Filter);
		glTextureParameteri(_Depth._Tex._Handle, GL_TEXTURE_WRAP_S, _Wrap);
		glTextureParameteri(_Depth._Tex._Handle, GL_TEXTURE_WRAP_T, _Wrap);
...
void Framebuffer::initFrameBuffers()
{
	...
	glGetIntegerv(GL_MAX_FRAMEBUFFER_WIDTH, &_MaxFramebufferSize.x);
	glGetIntegerv(GL_MAX_FRAMEBUFFER_HEIGHT, &_MaxFramebufferSize.y);
	glGetIntegerv(GL_MAX_FRAMEBUFFER_LAYERS, &_MaxFramebufferSize.z);

	SAT_DEBUG_LOG_SAFE("Framebuffer Stats: ");
	SAT_DEBUG_LOG_SAFE("Max Color Attachments:\t %d", _MaxColorAttachments);
	SAT_DEBUG_LOG_SAFE("Max Framebuffer Width:\t %d", _MaxFramebufferSize.x);
	SAT_DEBUG_LOG_SAFE("Max Framebuffer Height:\t %d", _MaxFramebufferSize.y);
	SAT_DEBUG_LOG_SAFE("Max Framebuffer Depth:\t %d", _MaxFramebufferSize.z);
}
...
{
	addDepthTarget();
	addColorTarget(GL_RGB8); // Albedo
	addColorTarget(GL_RGB8); // Normal
	addColorTarget(GL_R11F_G11F_B10F); // Emissive
	addColorTarget(GL_R8); // Roughness
	addColorTarget(GL_R8); // Metallic
	...
	textures.push_back(&_Color._Tex[3]);
	textures.push_back(&_Color._Tex[4]);
}
...
void Gbuffer::setDebugNames()
{
	...
	setOpenGLName(GL_TEXTURE, _Color._Tex[3].getID(), "GBuffer Roughness");
	setOpenGLName(GL_TEXTURE, _Color._Tex[4].getID(), "GBuffer Metallicness");
}
